...
```bash
git clone git@github.com:admiral-shlork/nixos-mini-slab.git
```
```bash
git clone https://github.com/admiral-shlork/nixos-mini-slab.git
```

5. To install the OS run the following command from the root folder of the cloned repository:

```bash
sudo nixos-install --root /mnt  --flake .#mini-slab
```
...
```bash
sudo nixos-rebuild switch --flake .#mini-slab
```
...
```
```bash
sudo nixos-rebuild boot --flake .#mini-slab
```
```bash
nh clean all --keep 2
```
